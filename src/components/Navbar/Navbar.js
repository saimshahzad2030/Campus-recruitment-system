                ...
                {isMenuOpen && (
                    <div className={`sm:hidden mt-4 bg-gray-100`}>
                        <div className={`flex flex-col items-center`}>
                            {links.map((link, index) => (
                                <Link key={link} href={`${link}`} className={`${menuLinksVisible ? 'opacity-100 translate-x-0' : 'opacity-0 translate-x-8'} text-xl text-gray-700 w-full ${style.link} text-center mt-4 transition-opacity duration-500 ease-in-out transform`}>
                                    {linkNames[index]}
                                </Link>
                            ))}
                        </div>
